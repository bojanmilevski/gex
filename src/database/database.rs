...
fn read_manifest(zip: &mut ZipArchive<File>) -> Result<Manifest> {
...
fn read_locales(manifest: &Manifest, zip: &mut ZipArchive<File>) -> Result<Vec<Locale>> {
	...
	let locales = locale_folders
		...
		.map(|locale| -> Result<Locale> {
			...
			Ok(Locale {
				description: Some(locale_file.extension_description.unwrap().message), // WEIRD
...
fn read_extensions_json(path: &PathBuf) -> Result<Addons> {
...
fn get_addon(path: &PathBuf, ext: &Extension) -> Result<Addon> {
	...
	let locales = read_locales(&manifest, &mut zip)?;
	let addon = generate_addon_info(&manifest, locales, &ext_path)?;
	Ok(addon)
}

pub async fn add_extension_to_database(flags: &Flags, ext: &Extension) -> Result<()> {
	let path = flags.profile.path.join("extensions.json");
	let mut addons = read_extensions_json(&path)?;
	let addon = get_addon(&flags.profile.path, &ext)?;
