...
async fn send_query(ext_slug: &String) -> Result<QueryResult> {
...
pub async fn query_extension(ext_slug: &String) -> Result<Extension> {
...
pub async fn query_extensions(ext_slug: &String) -> Result<Vec<Extension>> {
	let json = send_query(&ext_slug).await?;
	Ok(json.extensions)
}
