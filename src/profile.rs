...
impl TryFrom<CliConfiguration> for Profile {
	...
	fn try_from(configuration: CliConfiguration) -> Result<Self> {
		...
		if !path.join("extensions").exists() {
			std::fs::create_dir(path.join("extensions"))?;
		}
...
impl Profile {
	fn get_browser_path(browser: &str) -> Result<PathBuf> {
		...
		let path = match browser {
			...
			_ => {
				return Err(anyhow!(
					"{} is currently not supported. Please open an issue here: https://github.com/bojanmilevski/gex/issues",
					browser
				))
			}
		};
